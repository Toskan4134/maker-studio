...
**Colours** — every colour the editor uses, grouped by what it paints: **Surfaces** (backgrounds, panels, borders), **Text**, **Accent**, **Status** (danger/warning/success), **Map canvas**, **Event commands** (the colour coding of the command list in the event editor), **Script editor** (Ruby syntax highlighting) and **Individual commands** (one row per event command).

- **RPG Maker XP palette**, the button under **Individual commands**, fills every command with the classic RPG Maker XP ink — navy branches, teal variables, green comments, maroon battle commands. It dresses light *and* dark mode in one press (same colours, brightened so they read on a dark panel), and everything it writes is an ordinary colour you can then change or revert row by row.
- **Event commands** sets one colour per *kind* of command (all conditionals, all audio commands…), and **Individual commands** below it holds one row for every single command — Show Text, Conditional Branch, Play SE and the rest. A row you never touch shows the colour it inherits from its category, so changing the category still moves them all; change one row and that command alone leaves the group.

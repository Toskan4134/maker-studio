...
- **Linux** — make the AppImage executable (`chmod +x Maker.Studio_x86_64.AppImage`) and run it. It uses your system's WebKit: if it doesn't start, install `webkit2gtk-4.1` and `gtk3` (Arch) or `libwebkit2gtk-4.1-0` and `libgtk-3-0` (Debian/Ubuntu). The `.deb` installs with `sudo apt install ./Maker.Studio_amd64.deb`. The AppImage updates itself in place from the in-app updater (it keeps the same file path, so launchers keep working); the `.deb` does not — install the new package from the [releases page](https://github.com/Toskan4134/maker-studio/releases/latest).

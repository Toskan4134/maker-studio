...
Very tall images — a full tileset is hundreds of rows — open **fitted to the width**. When you reopen a tileset graphic with a saved tile selection, the preview starts on that tile block and its neighbours. A vertical bar beside the preview lets you drag through the full sheet; it stays in sync with the wheel, pan and zoom. Without a saved selection, it starts at the top.
...
**Every row says what the command does.** The text after a command's name spells out the options it was given, with names instead of numbers — `Scroll Map: Down, 3 tiles, speed 4`, `Change Items: 004: Potion + 2`, `Show Picture: #1 "title", opacity 255`. Switches, variables, maps, events and database entries are all named, so a page can be read top to bottom without opening a single command. Show and Move Picture rows stay short on purpose — name and opacity, plus the duration on Move — since the other options are almost always left at their defaults.

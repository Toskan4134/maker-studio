...
Click **Presets…** in the shadow configuration popup header to open the preset manager. Save the current direction / height / tint / opacity / offsets combination as a named preset and apply it later to any new shadow with one click. The dialog also supports rename, overwrite, and delete, and lets you select several presets (Ctrl/Shift+click, or click a folder's name) to move, export or delete them together. Use **Export…** to write the presets to a folder — the ones you selected, or all of them when nothing is selected — each as its own `ms_<name>_<index>_preset.json` file, and **Import…** to bring one or more preset files back in at once (for backups or sharing between machines). Presets are stored per-install (shared across all your projects) and remembered across sessions.

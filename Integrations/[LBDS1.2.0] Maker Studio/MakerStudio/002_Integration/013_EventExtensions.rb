...
#     plugin doesn't define — vanilla RMXP/Essentials/LBDS load the file fine
#     and simply never read the ivar. See ConditionTreeEditor.tsx / the
#     ConditionNode type / sim-conditions.ts on the editor side for the
#     matching shape and evaluator.
#
...
# maps untouched by these toggles behave exactly like vanilla RMXP/Essentials.
...
class Game_Event
  ...
  def eval_condition_node(node)
    ...
    when "variable"
      lt = variableIsLessThan?(node["id"], node["value"])
      node["negate"] ? lt : !lt
    when "self_switch"
  ...
  # Patches each page's condition so the base's own page-picking loop yields
  # the MS-correct page, then calls the original refresh (which includes
  # whatever other scripts chained on top — Events Utilities, etc.).
  # Restore in +ensure+ so exceptions don't corrupt condition state.
  ...
  def refresh
      ...
      @event.pages.each do |page|
        ...
        if c.self_switch_valid && c.instance_variable_get(:@self_switch_negate)
          ...
          if $game_self_switches[key] == true
            ...
            c.self_switch_ch = "Z"  # never set → reads as off
          else
            # negate + OFF → MS keeps; original skips. Force keep.
            patches << [c, :self_switch_valid, c.self_switch_valid]
    ...
    # register_event / unregister_event — part of the event-update culling some
    # LBDS 1.2.x cores added and others didn't — guard so the plugin loads
    # across the whole 1.2 line.
    if self.map && self.map.respond_to?(:register_event)
...
class Game_Map
  ...
  def passable?(x, y, dir, self_event = nil)
    ...
    __mkst_evtext__passable?(x, y, dir, self_event)
  end
end

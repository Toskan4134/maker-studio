...
class TilemapRenderer
  ...
  def create_extended_sprites_for_map(map, ext_data)
    ...
    visible_layers.each_with_index do |layer, layer_order|
      ...
      (layer["tiles"] || {}).each do |key, tile_data|
        ...
        # Z-ordering: check per-tile priority first (extra autotiles/cross-tileset),
        # then fallback to map tileset priorities
        if tile_data && tile_data["priority"]
          priority = tile_data["priority"].to_i
        elsif tile_data && tile_data["autotile_name"]
          # Extra autotile: look up from any tileset that has this autotile
          priority = resolve_autotile_priority(tile_data["autotile_name"], map)
        elsif tile_data && tile_data["tileset_id"]
          # Cross-tileset: use referenced tileset's priorities
          ts = $data_tilesets[tile_data["tileset_id"].to_i]
          priority = (ts && ts.priorities[tile_id]) ? ts.priorities[tile_id] : 0
        else
          priority = map.priorities[tile_id] || 0
        end
        sprite.ext_z_offset = z_offset
        if priority == 0
          sprite.z = 2 + z_offset  # Extended ground above shadow (z=1), matches editor render order
        else
          # Initial z uses map-y; refreshed each frame in update loop using
          # current screen-y so it tracks the player's screen-space z (which
          # is what determines whether the player passes above or below).
          sprite.z = (my * SOURCE_TILE_HEIGHT) +
                     (priority * SOURCE_TILE_HEIGHT) +
  ...
  def setup_extended_sprite_bitmap(sprite, tile_id, map, tile_data = nil)
    ...
    # Case 2: Cross-tileset reference
  ...
  def apply_native_tile_props_to_sprite(sprite, props, has_effects, map, screen_i, screen_j, tone, color)
    ...
    if autotile_name
      ...
      apply_native_visual_state(sprite, props, has_effects, 0, tone, color)
    ...
    if ts_id
      ...
      if ts
        ...
        if ts_bitmap && !ts_bitmap.disposed?
          ...
          priority = resolve_tile_priority(tile_id, props, map)
          sprite.set_bitmap(ts.tileset_name, tile_id, false, false, priority, ts_bitmap)
          if tile_id >= MakerStudio::TILESET_START_ID
            i = tile_id - MakerStudio::TILESET_START_ID
            src_col = i % MakerStudio::TILESET_TILES_PER_ROW
      ...
      apply_native_visual_state(sprite, props, has_effects, sprite.tile_id, tone, color)
      refresh_tile_coordinates(sprite, screen_i, screen_j)
      if sprite.ox != 0 || sprite.oy != 0
        sprite.x += (sprite.ox * sprite.zoom_x.abs).round
        sprite.y += (sprite.oy * sprite.zoom_y.abs).round
  ...
  #---------------------------------------------------------------------------
  # Adjust z of native tiles so shadow layering matches editor:
  #   passable ground (z=0) → shadow (z=1) → non-passable + source tiles (z=2)
  # Extended layer tiles already at z=2+ (set during creation).
  #---------------------------------------------------------------------------
  def adjust_ground_z_for_shadows
    ...
    $map_factory.maps.each do |map|
      ...
      next if shadows.empty? || shadows.none? { |s| s["visible"] }
      tileset = $data_tilesets[rpg_map.tileset_id]
      next unless tileset
      passages = tileset.passages
      priorities = tileset.priorities

      # Collect source tiles (cached per map to avoid rebuilding every frame).
      # Integer key = y * map_w + x; value = tile_id. Single Fixnum lookup per
      # hot-loop iteration instead of "#{x},#{y},#{tid}" string allocation.
      ...
      if !source_entry || source_entry[:map_w] != map_w
        ...
        shadows.each do |shadow|
          ...
          (shadow["sourceTiles"] || []).each do |st|
            sx = st["x"].to_i
            sy = st["y"].to_i
            source_keys[sy * map_w + sx] = st["tileId"].to_i
          end
        end
        source_entry = { map_w: map_w, keys: source_keys }
      ...
      # Skip off-screen maps — no shadow z-adjustment needed for maps
      # whose display area is completely outside the viewport.
      next unless map_screen_visible?(map, mdx, mdy)

      # Check all 3 native layers
      # Also load nativeProperties for per-tile passage overrides (extra autotiles)
      native_props = ext_data["nativeProperties"]
      map_h = map.height
      scr_w = @tiles_horizontal_count
      scr_h = @tiles_vertical_count
      MakerStudio::NATIVE_LAYERS.times do |layer|
        ...
        # Build a compact "row → cols with overridden passage" lookup keyed
        # by tile_y so the inner column loop can skip the layer_props hash
        # entirely when the row has no per-tile passage overrides.
        # Falls back to nil when no native_props on this layer.
        (0...scr_w).each do |i|
          tile_x = i + dx_tile
          ...
          (0...scr_h).each do |j|
            ...
            tid = sprite.tile_id
            next if tid <= 0
            pri = priorities[tid] || 0
            next unless pri == 0  # Only touch priority-0 tiles
            tile_y = j + dy_tile
            # Skip tiles outside this map's bounds — they belong to a
            # different connected map and must not receive z-adjustments
            # from this map's shadow data.
            next if tile_y < 0 || tile_y >= map_h
            src_tid = source_keys[tile_y * map_w + tile_x]
            is_source = src_tid == tid
            # Per-tile passage override (extra autotiles store passage in
            # nativeProperties). Fall back to tileset default.
            tile_passage = nil
            if layer_props
              entry = layer_props["#{tile_x},#{tile_y}"]
              tile_passage = entry["passage"] if entry
            end
            p = tile_passage != nil ? tile_passage : passages[tid]
            if is_source || (p && (p & 0x0F) == 0x0F)
              sprite.z = 2  # Exact source tile or non-passable: above shadow
            else
      ...
      visible_map_ids.each do |map_id, is_visible|
        ...
        rows.each do |row, sprites|
          ...
          sprites.each do |sprite|
            ...
            # Recompute z for priority>0 sprites in screen-y space
            pri = sprite.priority
            if pri && pri > 0
              sprite.z = (j * SOURCE_TILE_HEIGHT) +
                         (pri * SOURCE_TILE_HEIGHT) +

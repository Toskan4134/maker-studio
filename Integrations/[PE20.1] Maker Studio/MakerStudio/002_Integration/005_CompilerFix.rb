...
# v20.1 has no Translator module (the translation extractor arrived in v21.1),
# so only patch it where it exists — a plain "module Translator" would DEFINE an
# empty one here and then NameError on the alias.
if defined?(Translator) && Translator.respond_to?(:find_translatable_text_from_event_script)
  module Translator
    class << self
      # Alias once; redefine unconditionally so the override survives an mkxp F12
      # soft-reset (the engine re-evals Translator and restores the original; a
      # def inside the guard would be skipped on the reset's plugin re-run).
...
module Compiler
  class << self
    # Alias once; redefine unconditionally so these survive an mkxp F12 soft-reset
    # (the engine re-evals Compiler and restores the originals; defs inside the
    # guards would be skipped on the reset's plugin re-run and the sanitizers —
    # which guard the compiler against the editor's nil event scripts — would be
    # lost, so a recompile after reset could crash).

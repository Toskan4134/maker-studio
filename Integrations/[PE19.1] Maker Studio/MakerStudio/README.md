...
A Pokemon Essentials v19.1 plugin that provides an advanced external tilemap editor, replacing RPG Maker XP's native editor with a multi-layer system, per-tile visual effects, cross-tileset painting, unlimited autotiles, and a shadow system.
...
MakerStudio/
  ...
  002_Integration/
    001_Hooks.rb                  - Debug menu entries, lifecycle hooks, editor launcher
    002_OverlayRenderer.rb        - Sprite overlay above CustomTilemap: extended layers + shadows
    003_GameMapOverride.rb        - Game_Map collision/terrain patches for extended layers
    004_FogOverride.rb            - Fog / panorama / custom layer groups (scroll, hue, opacity)
    005_CompilerFix.rb            - Guards the event compiler against the editor's nil event scripts
    006_MapVersioning.rb          - Switch/variable-driven in-place map version swapping
    007_CustomSheetGrid.rb        - Non-4x4 character sheet grids
    008_FogCommands.rb            - Panorama/battleback overrides + fog event commands
    009_FrameCommands.rb          - Move Route frame actions
    010_MinimapOverride.rb        - Full-map preview in the debug map screens
  003_Editor/
...
5. v19.1 renders the map with `CustomTilemap`, which composites the native layers into bitmaps
   instead of keeping one sprite per tile. So `002_OverlayRenderer.rb` draws Maker Studio's own
   **sprite overlay layer** in the map viewport (extended layers, extra autotiles, cross-tileset
   tiles, per-tile effects, shadows), mirroring the engine's z bands so characters still walk in
   front of / behind the right tiles.
...
- **Author**: Toskan4134
- **Framework**: Pokemon Essentials v19.1
- **Editor**: Tauri v2, React 19, TypeScript, Rust

## License

This plugin is provided as-is for use with Pokemon Essentials v19.1 projects.

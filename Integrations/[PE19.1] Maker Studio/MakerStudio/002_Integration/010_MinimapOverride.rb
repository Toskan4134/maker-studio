...
module MakerStudio
  ...
  def build_minimap(mapid)
    ...
    echoln("MakerStudio ERROR: build_minimap failed: #{e.message}")
  ...
  #---------------------------------------------------------------------------
  # Extra (named) autotile -> 32x32 scratch. v19.1 has no AutotileExpander (that
  # class ships with the v20+ TilemapRenderer), so the raw strip is wrapped in a
  # TileDrawingHelper and assembled by its own autotile blitter.
  #
  # The BES build passes a nil tileset here; v19.1's TileDrawingHelper#initialize
  # calls `tileset.mega?` unguarded, so nil would NoMethodError (swallowed by
  # build_minimap's rescue -> the whole minimap silently falls back to the stock
  # one). A 1x1 scratch bitmap satisfies `mega?` (false) and is never blitted from
  # — only @autotiles is used for autotile assembly.
  #---------------------------------------------------------------------------
  def minimap_blit_extra_autotile(tmp, name, pattern, exp_cache, bes_cache, to_dispose)
    ...
    # CustomTilemap engines (v19.1 / BES): assemble through TileDrawingHelper.
    helper = bes_cache[name]
    unless helper
      bmp = pbGetAutotile(name) rescue nil
      return false unless bmp && !bmp.disposed?
      helper = TileDrawingHelper.new(Bitmap.new(1, 1), [bmp])

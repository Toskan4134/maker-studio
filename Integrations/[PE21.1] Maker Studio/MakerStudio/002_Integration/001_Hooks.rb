...
if $DEBUG
  ...
  # Command: Live Reload on/off
  MenuHandlers.add(:debug_menu, :ms_live_reload, {
...
def pbRefreshLiveMapData(map_id)
  ...
  if fresh_map && $game_map.data
    ...
    # Game_Map caches the tileset's @passages/@priorities/@terrain_tags (and
    # @tileset_name/@autotile_names) at setup. Reloading $data_tilesets above
    # does NOT refresh those cached ivars, so passage/priority/terrain edits
    # from the editor wouldn't apply on hot-reload or Continue — the renderer
    # reads map.priorities (the stale cached array) — until the tile happened to
    # already carry that priority at setup time. Re-read them from the freshly
    # loaded tileset so editor changes take effect immediately.
    ts = $data_tilesets[fresh_map.tileset_id] rescue nil
    if ts

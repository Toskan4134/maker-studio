...
# for the full rationale; this file is the same logic against the Pokémon
# Essentials BES core.
#
...
# BES has no variableIsLessThan?, so the condition tree's variable branch
# reads $game_variables directly.
#
...
class Game_Map
  ...
  # BES has at_coordinate? but its own passable? uses a plain x/y match; match
  # the core's position check so Block vetoes exactly the cells the base does.
  def passable?(x, y, d, self_event = nil)
